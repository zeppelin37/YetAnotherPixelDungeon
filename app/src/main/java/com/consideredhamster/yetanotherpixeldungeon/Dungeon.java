...
//    }
    ...
    public static void saveLevel(String fileName) throws IOException {
        ...
        OutputStream output = Game.instance.openFileOutput(Utils.format(fileName, depth), Game.MODE_PRIVATE);
        Bundle.write(bundle, output);
        output.close();
    }

    public static void saveCheckpoint() throws IOException {
        saveAll(); //to be sure the newly generated level exists on file
        copy(gameFile(hero.heroClass), "checkpoint.dat");
        for (int i = 0; i < 31; i++){
            copy(Utils.format(depthFile(hero.heroClass), i), Utils.format("checkpointLevel%d.dat", i));
        }
    }
    public static void copy(String inFile, String outFile) throws IOException {
       try {
           InputStream in = Game.instance.openFileInput(inFile);
           OutputStream out = Game.instance.openFileOutput(outFile, Game.MODE_PRIVATE);
           copy(in, out);
           out.close();
           in.close();
       } catch (IOException e){
;
    ...
    public static void saveAll() throws IOException {
        ...
        if (hero.isAlive()) {
            ...
            saveLevel(depthFile(hero.heroClass));
    ...
    public static void loadCheckpoint(HeroClass cl) throws IOException {
        copy( "checkpoint.dat", gameFile(cl));
        for (int i = 0; i < 31; i++){
                copy( Utils.format("checkpointLevel%d.dat", i),Utils.format(depthFile(cl), i));
        }
